...
export default new Endpoint({
    ...
    run: async (req, res, payload: { item: Item["id"] } & Pick<Loan, "quantity" | "note">) => {
        ...
        let reservations = await getRepository(Reservation).find({ user: req.user })
        //If Over Reservation Limi
        if (reservations.length > config.MAX_LOANS) throw {
            name: "Loan Limit Exceeded",
            message: `A limit of ${config.MAX_LOANS} reservation${config.MAX_LOANS === 1 ? "" : "s"} per user is imposed.  Please delete a reservation in order to reserve another item.`
        }
        //Check Item
        ...
        if (payload.quantity > item.quantity) throw {
            name: "Insufficient Resources",
            message: `Only ${payload.quantity} of the requested item ${payload.quantity === 1 ? "is" : "are"} available.`
        }
        //Check if User has an Existing Reservation
        let userReservation = item.reservations.find(r => r.user.id === req.user.id)
        if (userReservation) throw {
            name: "Item Reserved",
