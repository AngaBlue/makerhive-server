...
export default new Endpoint({
    ...
    run: async (req, res, payload: { item: Item["id"] } & Pick<Loan, "quantity" | "note">) => {
        ...
        let loans = await getRepository(Loan).find({
            user: req.user,
            returned: IsNull()
        });
        //If Over Loan Limit
        if (loans.length > config.MAX_LOANS)
            throw {
                name: "Loan Limit Exceeded",
                message: `A limit of ${config.MAX_LOANS} loan${
                    config.MAX_LOANS === 1 ? "" : "s"
                } per user is imposed.  Please return a loan to the Makerspace in order to loan another item.`
            };
        //If Has an Overdue Loan
        let overdue = loans.filter(
            (loan) => loan.borrowed.getUTCMilliseconds() + (config.LOAN_LENGTH as number) < Date.now()
        );
        if (overdue.length > 0)
            throw {
                name: "Loan Overdue",
                message: `You have ${overdue.length} item ${
                    overdue.length === 1 ? "" : "s"
                }.  Please return overdue loans to the Makerspace in order to loan another item`
            };
        //Check Item
        ...
        let reservations = item.reservations
            .slice(0, userReservation === -1 ? item.reservations.length : userReservation)
        ...
        if (available - reservations < payload.quantity)
            throw {
                name: "Item Reserved",
        ...
        if (userReservation !== -1) {
            ...
            if (reservation.quantity < 1) await getRepository(Reservation).delete(reservation.id);
            else await getRepository(Reservation).save(reservation);
        }
        //Borrow
        ...
        return await getRepository(Loan).findOne({
            where: { id: loan.id },
            relations: ["item"]
        });
    }
